...
These variables configure the `goosed-plus` server process. They are most often used when [running `goosed-plus` on a remote machine](/docs/guides/remote-goose-server) and connecting goose Desktop to it, but they apply to any `goosed-plus` invocation.
...
| `GOOSE_TLS` | Enable TLS with a self-signed certificate. Required when connecting goose Desktop to a remote `goosed-plus`. | `true`, `false` | `true` |
...
```bash
# Start a goosed-plus server reachable on the local network over TLS
...
When TLS is enabled, `goosed-plus` prints a `GOOSED_CERT_FINGERPRINT=...` line on startup. Clients (such as goose Desktop) need this fingerprint to verify the self-signed certificate. See [Running a Remote goose Server](/docs/guides/remote-goose-server) for the full setup.
