...
[Nebius Token Factory](https://tokenfactory.nebius.com/) provides hosted open models through an OpenAI-compatible API at `https://api.tokenfactory.nebius.com/v1`. Models are discovered dynamically from the provider's `/v1/models` endpoint, with a static fallback list in [nebius.json](https://github.com/aaif-goose/goose/blob/main/crates/goose/src/providers/declarative/nebius.json). To use Nebius with goose, create an API key at [tokenfactory.nebius.com](https://tokenfactory.nebius.com/project/api-keys).
