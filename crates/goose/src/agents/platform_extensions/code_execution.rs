...
impl CodeExecutionClient {
    ...
    async fn handle_execute_bash(
        ...
        Ok(vec![Content::text(output.to_string())])
    }
    ...
    async fn handle_execute_typescript(
        ...
        let dispatch_token = cancellation_token.child_token();
        let registry =
            self.build_callback_registry(session_id, &code_mode, dispatch_token.clone())?;
...
impl McpClientTrait for CodeExecutionClient {
    ...
    async fn call_tool(
        ...
        let result = match name {
            ...
            "execute_typescript" => {
                self.handle_execute_typescript(session_id, arguments, cancellation_token)
                    .await
            }
