...
#[test]
fn test_recipe_parsing_error_includes_raw_content() {
    let content = "version: \"1.0.0\"\ntitle: [unterminated".to_string();
    let err = RecipeError::RecipeParsing {
        source: anyhow::anyhow!("invalid type: sequence, expected a string"),
        content: content.clone(),
    };

    let rendered = err.to_string();
    assert!(rendered.contains("Recipe parsing failed"));
