...
fn decode_audio_simple(audio_data: &[u8]) -> Result<Vec<f32>> {
    ...
    let audio_params = match track.codec_params.as_ref() {
        ...
        _ => anyhow::bail!("No audio codec parameters in audio track"),
    };
...
fn audio_buffer_to_f32(buffer: &GenericAudioBufferRef) -> Vec<f32> {
    match buffer {
...
fn resample_audio(data: &[f32], from_rate: u32, to_rate: u32) -> Result<Vec<f32>> {
    ...
    let (_, nbr_out) = resampler.process_all_into_buffer(&input, &mut output, input_len, None)?;

    let mut resampled = output_data.into_iter().next().unwrap();
    resampled.truncate(nbr_out);

    tracing::debug!(output_samples = resampled.len(), "resampling complete");
    Ok(resampled)
}
