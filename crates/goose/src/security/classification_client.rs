...
impl ClassificationClient {
    ...
    /// Normalize and chunk `text` before classifying, then return the
    /// highest confidence across chunks. Reduces token cost and dilution
    /// from verbose, repetitive tool output (e.g. long directory listings)
    /// that would otherwise crowd out the actually distinctive content
    /// within the classifier's fixed token window.
    pub async fn classify_with_normalization(&self, text: &str) -> Result<f32> {
        ...
        let results: Vec<Result<f32>> = stream::iter(chunks)
            ...
            .buffer_unordered(3) // classify up to 3 chunks concurrently
            .collect()
            .await;
