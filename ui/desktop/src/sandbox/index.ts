...
function materialiseBlockedList(): string {
  const runtimePath = path.join(sandboxDir, 'blocked.txt');
  if (!fs.existsSync(runtimePath)) {
    fs.mkdirSync(sandboxDir, { recursive: true });
    const content = fs.readFileSync(bundledPath('blocked.txt'), 'utf-8');
    fs.writeFileSync(runtimePath, content);
    log.info('[sandbox] Materialised blocked.txt');
  }
  return runtimePath;
}
...
export async function ensureProxy(): Promise<ProxyInstance> {
  ...
  const blockedPath = materialiseBlockedList();
