...
export async function checkBlocked(
  ...
  ldCache:
    | { get(key: string): boolean | undefined; put(key: string, value: boolean): void }
    | undefined,
...
export async function startProxy(options: ProxyOptions = {}): Promise<ProxyInstance> {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(listenPort, '127.0.0.1', () => {
      ...
      if (launchDarkly) {
        log.info(
          `[sandbox-proxy] LaunchDarkly: enabled (user=${
            launchDarkly.username || os.userInfo().username
          }, flag=egress-allowlist, cache=${launchDarkly.cacheTtlSeconds ?? 3600}s)`
        );
      }
